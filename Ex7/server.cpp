...
// }
    while (1) {
        ...
        int* client_fd = malloc(sizeof(int)); // נדרש כדי להעביר לפונקציית thread
        if (!client_fd) {
            perror("malloc");
            continue;
        }
        ...
        *client_fd = accept(listener_fd, (struct sockaddr*)&client_addr, &addr_size);
        ...
        if (pthread_create(&client_thread, NULL, handle_client_data, client_fd) != 0) {
        ...
        // לא צריך להצטרף כי אנחנו לא רוצים לחכות להם
